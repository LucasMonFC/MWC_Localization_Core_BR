...
- Assembly `HintPath` entries in [MWC_Localization_Core.csproj](MWC_Localization_Core.csproj) reference `$(GameDir)\*.dll`. `$(GameDir)` is resolved by [Directory.Build.props](Directory.Build.props), in this order:
  1. `-p:GameDir=...` on the MSBuild command line (set by `build.ps1` after detection)
  2. `%MWC_GAME_DIR%` environment variable
  3. Steam install root from `HKCU\Software\Valve\Steam\SteamPath`, probed for `...\steamapps\common\My Winter Car\mywintercar_Data\Managed`
  ...
  `build.ps1` does the richer detection: reads `steamapps\libraryfolders.vdf` for every Steam library root, then locates MWC by App ID **4164420** (parses `installdir` from `appmanifest_4164420.acf`). VS 2022 only sees the props-file fallbacks — set `MWC_GAME_DIR` in your user env vars if you have a non-default install and use VS directly.
- The `PostBuildEvent` block copies the resulting DLL/PDB into `$(GameModsDir)` (derived from `$(GameDir)`) and optionally to `MSCMODSFOLDER` / `MWCMODSFOLDER` if those MSBuild properties are set. It also invokes `debug.bat` in that folder when present — that's MSCLoader's auto-debug hook. Releases skip the PDB and the debug.bat call.
...
- Translation lookups go through `TranslationDictionary` ([TranslationDictionary.cs](TranslationDictionary.cs)): `TryGetExact(source, path, out value)` for direct lookup (with LRU + already-normalized fast path) and `TryMatchPattern(source, path)` for pattern fallback. Always pass the current GameObject path so path-scoped entries (see below) resolve correctly; pass `null` only when the caller truly has no path context. Don't reach into the underlying `Dictionary<string, string>` directly — normalization + caching happens inside.
- Path-scoped translation files: entries registered via `TranslationDictionary.AddScoped(scopeId, pathPrefixes, entries)` are only visible to lookups whose `path` starts with one of `pathPrefixes` (ordinal `StartsWith`); off-scope lookups skip them entirely. Used today for `translate_magazine.txt`, which is restricted to `CARPARTS/PARTSYSTEM/PostSystem/` and `Sheets/YellowPagesMagazine/` (see `MagazineScopePathPrefixes` in [MWC_Localization_Core.cs](MWC_Localization_Core.cs)) so short tokens like `h.` don't leak into unrelated text. Lookup order when in scope: scoped dict → global dict.

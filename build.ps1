...
function Find-MwcGameDir {
    foreach ($lib in Get-SteamLibraryRoots) {
        $manifest = Join-Path $lib "steamapps\appmanifest_$MwcAppId.acf"
        if (-not (Test-Path $manifest)) { continue }

        $m = [regex]::Match((Get-Content -Raw -LiteralPath $manifest), '"installdir"\s+"([^"]+)"')
        if (-not $m.Success) { continue }

        $candidate = Join-Path $lib "steamapps\common\$($m.Groups[1].Value)\mywintercar_Data\Managed"
...
$project = Join-Path $PSScriptRoot 'MWC_Localization_Core.csproj'

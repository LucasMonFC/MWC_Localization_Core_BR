...
namespace MSC_Localization_Core
{
    public sealed class TextureReplacementSurface : ITranslationSurface
    {
        ...
        private int ApplyLoadedTextures()
        {
            ...
            applied += InstallRallySheetTextureHooks(loadedSceneName);
        ...
        private int ApplyMaterialProperty(Material material, string propertyName)
        {
            try
            {
        ...
        private int InstallRallySheetTextureHooks(string sceneName)
        {
            if (sceneName != GameSceneName || !HasRallySheetDynamicTextures())
                return 0;
            ...
            for (int i = 0; i < RallySheetTriggerPaths.Length; i++)
            {
                ...
                applied += HookRallySheetFsmTextures(obj);
            }
        ...
        private int HookRallySheetFsmTextures(GameObject sheetBackground)
        {
            ...
            for (int i = 0; i < fsms.Length; i++)
            {
                ...
                {
                    SetMaterialTexture action = state.Actions[actionIndex] as SetMaterialTexture;
                    if (action == null)
                        continue;
            ...
            return hooked + ApplyRallySheetTextures(sheetRoot.GetComponentsInChildren<Renderer>(true));
        }

        private int ApplyRallySheetTextures(Renderer[] renderers)
        {
            ...
            for (int i = 0; i < renderers.Length; i++)
            {
                ...
                for (int j = 0; j < materials.Length; j++)
                {
                    ...
                    if (IsUnityObjectNull(currentTexture) || !IsRallySheetDynamicTexture(currentTexture.name))
                        continue;

                    applied += ApplyMaterialProperty(material, RallySheetMainTextureProperty);
                }
            }
        ...
        private static GameObject FindRallySheetRoot(GameObject sheetBackground)
        {
        ...
        private bool HookRallySheetTextureAction(SetMaterialTexture action)
        {
            if (action.texture == null || IsUnityObjectNull(action.texture.Value))
                return false;

            Texture currentTexture = action.texture.Value;
            if (!IsRallySheetDynamicTexture(currentTexture.name))
                return false;

            Texture2D replacement;
            string matchedName;
            if (!TryGetReplacementTexture(currentTexture.name, out replacement, out matchedName) || IsUnityObjectNull(replacement))
                return false;
            ...
            CopyTextureSettings(currentTexture, replacement);
            action.texture.Value = replacement;
            matchedTextureNames.Add(matchedName);
            return true;
        }

        private static bool EnsureFsmInitialized(PlayMakerFSM fsm)
        {
        ...
        {
            string normalizedTextureName = NormalizeTextureName(textureName);
            for (int i = 0; i < RallySheetDynamicTextureNames.Length; i++)
            {
                if (string.Equals(normalizedTextureName, RallySheetDynamicTextureNames[i], StringComparison.OrdinalIgnoreCase))
                    return true;
            }

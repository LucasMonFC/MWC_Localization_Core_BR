    ...
    public sealed class TextureReplacementSurface : ITranslationSurface
    {
        ...
        private void LoadReplacementTextures(string sceneName)
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                Texture2D texture = LoadPng(file, textureKey);
        ...
        private string[] GetTextureFilesForScene(string sceneName)
        {
            ...
            {
                string[] menuFilesOnDisk = Directory.GetFiles(texturesFolder, "*.png", SearchOption.TopDirectoryOnly);
                List<string> menuFiles = new List<string>();
                for (int i = 0; i < menuFilesOnDisk.Length; i++)
                {
                    string textureName = Path.GetFileNameWithoutExtension(menuFilesOnDisk[i]);
                    if (IsMenuOnlyTexture(textureName))
                        menuFiles.Add(menuFilesOnDisk[i]);
                }

                return menuFiles.ToArray();
            }

            string[] files = Directory.GetFiles(texturesFolder, "*.png", SearchOption.TopDirectoryOnly);

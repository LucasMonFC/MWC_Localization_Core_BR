...
namespace MWC_Localization_Core
{
    ...
    public class MagazineTextHandler
    {
        ...
        private HashSet<TextMesh> translatedMagazineTexts = new HashSet<TextMesh>();

        public MagazineTextHandler()
        {
        }
        ...
        public void LoadMagazineTranslations(string translationPath)
        {
            ...
            {
                CoreConsole.Warning($"Magazine translation file not found: {translationPath}");
                return;
            }
            ...
            try
            {
                ...
                CoreConsole.Print($"Loaded {magazineTranslations.Count} magazine translations");
            }
            catch (System.Exception ex)
            {
                CoreConsole.Error($"Failed to load magazine translations: {ex.Message}");
            }
        }
        ...
        public bool HandleMagazineText(TextMesh textMesh)
        {
            ...
            // Check if this text needs special processing FIRST (before HashSet check)
            bool needsSpecialProcessing = 
                textMesh.text.Split(',').Length == 3 || 
                (textMesh.text.StartsWith("h.") && textMesh.text.Contains(",- puh."));

            // Only skip if already processed AND doesn't need special processing
            if (!needsSpecialProcessing && translatedMagazineTexts.Contains(textMesh))
            {
                return true; // Already handled
            }

            CoreConsole.Print($"[Magazine] Processing text: '{textMesh.text}' (special: {needsSpecialProcessing})");
            ...
            if (textMesh.text.Split(',').Length == 3)
            {
                ...
                translatedMagazineTexts.Add(textMesh);
                return true; // Handled
            }
            ...
            CoreConsole.Print($"[Magazine] No translation found for: '{textMesh.text}' (key: '{key}')");
            translatedMagazineTexts.Add(textMesh);
        ...
        {
            CoreConsole.Print($"[Magazine] Translating comma-separated words: '{textMesh.text}'");
        ...
        {
            CoreConsole.Print($"[Magazine] Translating price/phone line: '{textMesh.text}'");
            try
            {
            ...
            {
                CoreConsole.Warning($"Failed to parse magazine price/phone line: {textMesh.text} - {ex.Message}");
            }
        }
